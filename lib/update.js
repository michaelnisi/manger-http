...
const {crash, latency, createServiceError, milliseconds} = require('./meta');
...
function update(_req, _res, opts, cb = () => {}) {
  ...
  manger.flushCounter((counterError, feedCount) => {
    ...
    manger.update((updateError, urls) => {
      ...
      if (urls.length > 0) {
        const feeds = urls.length;
        const minutes = milliseconds(latency(t, null) / 1000 / 60).toFixed(2);

        log.warn({feeds, minutes}, 'updated');
      } else {
        log.warn('no updates');
      }
    });
