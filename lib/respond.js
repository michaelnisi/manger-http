...
const {latency, crash, createServiceError} = require('./meta');
...
function respondAfterError(error, {req, res, payload, time, log}) {
  ...
  switch (statusCode) {
    case 200:
      log.warn(
        {method: req.method, url: req.url, error: message},
        'tolerating error',
      );
      ...
      if (!payload) {
        respond(
          ...
          JSON.stringify({
            error: message,
            reason: 'uncritical error without payload',
          }),
          time,
          log,
        );
    ...
    case 502:
      ...
      respond(
        ...
        JSON.stringify({
          error: message,
          reason: 'uncritical error with payload',
        }),
        time,
        log,
      );
